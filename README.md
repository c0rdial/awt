...
The supplied Sanity project ID is `pbnloqtf`, and this setup uses its `production` dataset. Copy `.env.example` to `.env` only when you need to override either value. The dataset currently has no published documents. The website shows layout placeholders until content is published.
